// Adapted from https://github.com/fitzgen/generational-arena/blob/master/src/lib.rs
// Modifications:
// - `Index` size is reduced to 64 bits, using 2 u32s instead of 2 u64s
// - `Index` is now `#[repr(C)]`, allowing it to be used in FFI
...
impl<T> Arena<T> {
    ...
    pub fn get2_mut(&mut self, i1: Index, i2: Index) -> (Option<&mut T>, Option<&mut T>) {
        ...
        if i1.index as usize >= len {
            return (None, self.get_mut(i2));
        } else if i2.index as usize >= len {
            return (self.get_mut(i1), None);
        }
